...
class Environment:
    ...
    def reset(self):
        ...
        self.final_rate = []
        self.epoch_events = {
            "GAME START": self.simenv.event(),
        }
        for idx in range(self.nUAV):
            self.epoch_events["UAVSREADY_{}".format(idx)] = self.simenv.event()
        return self.step([-1])
            ...
            for j in range(26):
                ...
                for k in range(4):
                    if bs_state[k] == 1:
                        if bs_state[k] == 1:
                            bs_dist.append(
                                math.sqrt((height_bs ** 2) + np.sum(self.list_sqadd(bs_coord[k], user_coord))))
                ...
                for nuav in range(self.uavs):
                    ...
                    if uav2user_dist < 60:
                        ...
                        power_uav[count_idx] = trans_power * ((4 * math.pi * freq / c) ** (-2)) * (
                                    1 / uav2user_dist) * (
                                                       uav_los + uav_nlos) ** (-1)
                        break
                # 计算uav间距离
                    ...
                    for l in range(len(bs_dist)):
                        if bs_dist[l] != min(bs_dist):
                            power_bs_interference.append(
                                (trans_power * gt * gr * (c / freq) ** 2) / 4 * math.pi * bs_dist[l] * 10)
                        else:
